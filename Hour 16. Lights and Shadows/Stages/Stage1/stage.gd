...
func _ready() -> void:
	randomize()
	$player.connect("destroyed", Callable(self, "_on_player_destroyed"))
...
func add_asteroid():
	var asteroid_instance2 = asteroid_scene.instantiate()
	asteroid_instance2.position = Vector2(SCREEN_WIDTH + 8, randf_range(0, SCREEN_HEIGHT))
	asteroid_instance2.connect("score", Callable(self, "_on_player_score"))
